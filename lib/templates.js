...
const SITE = {
  ogImage: "",                                  // 分享缩略图：留空→用文章首图→再无则纯文字卡片（可填 /uploads/xx.png 或完整网址）
...
/* 把相对路径补成绝对 URL（OG / Twitter / canonical 都要求绝对地址） */
function absUrl(origin, u) {
  if (!u) return "";
  if (/^https?:\/\//i.test(u)) return u;
  return (origin || "") + (u.charAt(0) === "/" ? u : "/" + u);
}
/* 取正文里的第一张图片地址（用作文章分享图） */
function firstImage(content) {
  const m = /!\[[^\]]*\]\(([^)\s]+)\)/.exec(String(content || ""));
  return m ? m[1] : "";
}
/* 生成 SEO / 社交分享标签：canonical + Open Graph + Twitter Card + 可选 JSON-LD / noindex */
function seoHead(seo) {
  seo = seo || {};
  const out = [];
  if (seo.noindex) out.push('<meta name="robots" content="noindex,nofollow" />');
  if (seo.url) out.push('<link rel="canonical" href="' + esc(seo.url) + '" />');
  out.push('<meta property="og:site_name" content="' + esc(SITE.name) + '" />');
  out.push('<meta property="og:type" content="' + esc(seo.type || "website") + '" />');
  if (seo.title) out.push('<meta property="og:title" content="' + esc(seo.title) + '" />');
  if (seo.desc) out.push('<meta property="og:description" content="' + esc(seo.desc) + '" />');
  if (seo.url) out.push('<meta property="og:url" content="' + esc(seo.url) + '" />');
  if (seo.image) out.push('<meta property="og:image" content="' + esc(seo.image) + '" />');
  out.push('<meta name="twitter:card" content="' + (seo.image ? "summary_large_image" : "summary") + '" />');
  if (seo.title) out.push('<meta name="twitter:title" content="' + esc(seo.title) + '" />');
  if (seo.desc) out.push('<meta name="twitter:description" content="' + esc(seo.desc) + '" />');
  if (seo.image) out.push('<meta name="twitter:image" content="' + esc(seo.image) + '" />');
  if (seo.jsonld) out.push('<script type="application/ld+json">' + JSON.stringify(seo.jsonld).replace(/</g, "\\u003c") + "</script>");
  return out.join("\n");
}
...
function shell(opts) {
  ...
  const desc = opts.desc || SITE.bannerSub;
  const seo = Object.assign({ title: fullTitle, desc: desc }, opts.seo || {});
  return [
    "<!DOCTYPE html>",
...
function home(posts, page, origin) {
  ...
  return shell({ title: "", active: "home", banner: bannerBig(SITE.bannerTitle, SITE.bannerSub), body: body,
    seo: { url: absUrl(origin, "/"), type: "website", image: absUrl(origin, SITE.ogImage) } });
}

function archive(posts, origin) {
  const byYear = {};
  ...
  return shell({ title: "归档", active: "archive", banner: bannerSmall("归档", "共 " + posts.length + " 篇文章"), body: body,
    seo: { url: absUrl(origin, "/archive"), type: "website", image: absUrl(origin, SITE.ogImage) } });
}
...
function about(origin) {
  ...
  return shell({ title: "关于", active: "about", banner: bannerSmall("关于"), body: body,
    seo: { url: absUrl(origin, "/about"), type: "website", image: absUrl(origin, SITE.ogImage) } });
}

function postPage(post, origin) {
...
function notFound() {
  ...
  return shell({ title: "404", active: "", banner: bannerSmall("404", "页面不存在"), body: body, seo: { noindex: true } });
}
...
function adminShell(bodyInner, opts) {
  ...
  return shell({ title: opts.title || "管理后台", active: "", banner: bannerSmall(opts.banner || "管理后台", opts.sub || ""), body: body, tailScript: opts.tailScript, seo: { noindex: true } });
}
...
function filterListPage(kind, value, posts, page, origin) {
  const pg = paginate(posts, page);
  ...
  return shell({ title: label + "「" + value + "」", active: "",
    ...
    seo: { url: absUrl(origin, base), type: "website", image: absUrl(origin, SITE.ogImage) } });
}
function tagPage(tag, posts, page, origin) { return filterListPage("tag", tag, posts, page, origin); }
function categoryPage(cat, posts, page, origin) { return filterListPage("category", cat, posts, page, origin); }
...
function sitemap(posts, origin) {
  const s = store.stats();
  const urls = [origin + "/", origin + "/archive", origin + "/about"];
  posts.forEach(function (p) {
    urls.push({ loc: origin + "/post/" + encodeURIComponent(p.slug), lastmod: String(p.updated || p.date || "").slice(0, 10) });
  });
  Object.keys(s.cats).forEach(function (c) { urls.push(origin + "/category/" + encodeURIComponent(c)); });
  Object.keys(s.tags).forEach(function (t) { urls.push(origin + "/tag/" + encodeURIComponent(t)); });
  const body = urls.map(function (u) {
    if (typeof u === "string") return "<url><loc>" + xmlEsc(u) + "</loc></url>";
    return "<url><loc>" + xmlEsc(u.loc) + "</loc>" + (u.lastmod ? "<lastmod>" + xmlEsc(u.lastmod) + "</lastmod>" : "") + "</url>";
  }).join("");
  return '<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">' + body + "</urlset>";
}

module.exports = { home, archive, about, postPage, notFound, login, dashboard, editor, tagPage, categoryPage, rssFeed, sitemap };
